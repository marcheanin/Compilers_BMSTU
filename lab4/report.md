...
# Цель работы
Целью данной работы является приобретение навыка реализации лексического анализатора на объектно-ориентированном языке 
...
public:
    ...
    Token(DomainTag tag, Position start, Position follow) : tag(tag), coords(std::move(start), std::move(follow)) {}
...
В чём был баг: как я и сказал на паре, лексер неправильно восстанавливался после обнаружения ошибки, и действительно, 
в методе raise_error() было достаточно добавить break после обнаружения перехода на новую строку, и еще в считывании
...
# Вывод
В этой лабораторной работе я разработал лексический анализатор по всем правилам разработки такого анализатора, на 
своём любимом языке с++. Мне действительно показалось интересным и полезным построить всю структуру классов для токенов
