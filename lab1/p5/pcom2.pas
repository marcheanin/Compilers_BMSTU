...
const 
   ...
   maxres     = 40;  { number of reserved words }
...
type                                                        (*describing:*)
     ...
     symbol = (ident,intconst,realconst,stringconst,notsy,mulop,addop,relop,
               ...
               procsy,setsy,packedsy,arraysy,recordsy,filesy,beginsy,begin2sy,ifsy,
               casesy,repeatsy,whilesy,forsy,withsy,gotosy,endsy,end2sy,elsesy,untilsy,
...
1:  fcp := lcp
  ...
  procedure block(fsys: setofsys; fsy: symbol; fprocp: ctp);
    ...
    (*record*)      if sy = recordsy then
                    begin insymbol;
                      ...
                      pshstc(lsp);
                      if (sy = endsy) or (sy = end2sy) then insymbol else error(13)
                    end
                  else
          ...
          repeat block(fsys,semicolon,lcp);
            if sy = semicolon then
              begin if prtables then printtables(false); insymbol;
                if not (sy in [beginsy,begin2sy,procsy,funcsy]) then
                  begin error(6); skip(fsys) end
              end
            else error(14)
          until (sy in [beginsy,begin2sy,procsy,funcsy]) or eof(input);
      ...
      procedure statement(fsys: setofsys);
        ...
        begin expression(fsys + [ofsy,comma,colon]);
          ...
          repeat
            ...
            if not(sy in [semicolon,endsy,end2sy]) then
              begin
        ...
        if sy in statbegsys + [ident] then
          begin
            case sy of
              ...
              begin2sy: begin insymbol; compoundstatement end;
            ...
            if not (sy in [semicolon,endsy,end2sy,elsesy,untilsy]) then
              begin error(6); skip(fsys) end
          end
      end (*statement*) ;
    ...
    begin (*body*)
      ...
      addlvl;
      repeat
        repeat statement(fsys + [semicolon,endsy,end2sy])
        until not (sy in statbegsys);
        test := sy <> semicolon;
        if not test then insymbol
      until test;
  ...
  begin (*block*)
    ...
    repeat
      ...
      if (sy <> beginsy) or (sy <> begin2sy) then
        begin error(18); skip(fsys) end
    until (sy in statbegsys) or eof(input);
    dp := false;
    if (sy = beginsy) or (sy = begin2sy) then insymbol else error(17);
  ...
  begin
    ...
    blockbegsys := [labelsy,constsy,typesy,varsy,procsy,funcsy,beginsy,begin2sy];
    selectsys := [arrow,period,lbrack];
    facbegsys := [intconst,realconst,stringconst,ident,lparent,lbrack,notsy,nilsy];
    statbegsys := [beginsy,begin2sy,gotosy,ifsy,whilesy,repeatsy,forsy,withsy,casesy];
  end (*initsets*) ;
  ...
  procedure inittables;
    ...
    begin
      ...
      rw[34] := 'function '; rw[35] := 'procedure'; rw[36] := '{{       ';  rw[37] := '}}       ';
    ...
    begin
      ...
      rsy[34] := funcsy;    rsy[35] := procsy;    rsy[34] := begin2sy; rsy[35] := end2sy;
